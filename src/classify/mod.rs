...
pub fn from_config(
    ...
    debounce_ms: u64,
) -> Box<dyn StateClassifier> {
    match config {
        ClassifierConfig::Claude => Box::new(claude::ClaudeClassifier::new(
            idle_threshold_ms,
            debounce_ms,
        )),
        ClassifierConfig::Simple => Box::new(simple::SimpleClassifier::new(idle_threshold_ms)),
...
mod tests {
    ...
    fn from_config_claude() {
        let c = from_config(&ClassifierConfig::Claude, 3000, 200);
    ...
    fn from_config_simple() {
        let c = from_config(&ClassifierConfig::Simple, 3000, 200);
        assert_eq!(c.state(), ProcessState::Idle);
    ...
    #[test]
    fn from_config_none() {
        let c = from_config(&ClassifierConfig::None, 3000, 200);
        assert_eq!(c.state(), ProcessState::Idle);
    ...
    fn classifier_orthogonality() {
        let mut simple = from_config(&ClassifierConfig::Simple, 3000, 200);
        ...
        let mut claude = from_config(&ClassifierConfig::Claude, 3000, 0);
